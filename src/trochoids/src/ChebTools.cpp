...
//        std::cout << coeff_shift_xmax << std::endl;
    ...
    */
    ...
    //Based on kcachegrind, this is one of the functions that real_roots2 is taking most of it's time in 
    ...
    */
    ...
    // Two different approaches to calculating the roots -> 
    std::vector<double> ChebyshevExpansion::real_roots2(bool only_in_domain) const {
        ...
        // y_clenshaw_xscaled is one of the functions that it spends a lot of time in 
        Eigen::VectorXd xscaled = get_CLnodes(Ndegree_scaled), yy = y_Clenshaw_xscaled(xscaled);
        double ytol = 1e-5*(yy.maxCoeff()-yy.minCoeff()); //this could probably be changed to our epsilon 
        ...
        for (auto i = 0; i+2 < Ndegree_scaled+1; i += 2){
            ...
            if (D >= 0) {
                ...
                if (a == 0){
                    ...
                    // Numerically stable method for solving quadratic (could this be optmized?)
                ...
                auto secant = [e](double a, double ya, double b, double yb, double yeps = 1e-5, double xeps = 1e-5) {
                    auto c = b - yb*(b - a) / (yb - ya);
                    auto yc = e.y_Clenshaw_xscaled(c); //Another call of that same function 
                    for (auto i = 0; i < 50; ++i){
                        if (yc*ya > 0) {
                            a=c; ya=yc;
                        }
                        ...
                        c = b - yb*(b - a) / (yb - ya);
                        yc = e.y_Clenshaw_xscaled(c); //one more time 
                    }
                    return c;
                };
